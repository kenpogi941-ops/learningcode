...
// We also check if the POSTGRES_URL is actually there to avoid crashes
...
export async function query(command: string, params: unknown[] = []) {
  if (isProd) {
    try {
      ...
      console.error("Critical Cloud DB Error, falling back to temp SQLite:", e);
      // If Postgres fails, we fall through to the SQLite logic below
    }
  }

  // --- FALLBACK / LOCAL SQLITE LOGIC ---
  const sqlite3 = (await import('sqlite3')).default;
  const { open } = await import('sqlite');

  if (!sqliteDb) {
    // In Vercel (Prod) but without Postgres, we use /tmp
